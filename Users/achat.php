    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        foreach ($ids as $id) {
            ...
            for ($i = 0; $i < $quantite; $i++) {
                ...
                $resultinfo = mysqli_query($bdd, "SELECT * FROM compte WHERE compte.idCompte = $idCompte");
                ...
                if ($product) {
                    ...
                    $resultBENEF = mysqli_query($bdd, "SELECT idBeneficiaire FROM beneficiaire WHERE beneficiaire.idBeneficiaire = '$idCompte'");

                    if (mysqli_num_rows($resultBENEF) > 0) {
                        //c'est un beneficiaire
                        ...
                        //Créer aussi un bénéficiaire idCompte s'il n'y en a pas
                        mysqli_query($bdd, "INSERT INTO beneficiaire (idBeneficiaire, nom, prenom, email) VALUES ('$idCompte', '{$row['nom']}', '{$row['prenom']}', '{$row['email']}')");
                        // Insertion des données d'achat dans la table "achat"
                        $query = "INSERT INTO achat VALUES (NULL, NOW(), '$cleAchat', $idCompte, $idCompte, '$id')";
